...
use crate::{
    character::{Gender, AdminLevel, Race},
    data::{EOChar, Serializeable, StreamBuilder, StreamReader, EOInt},
};
...
impl Serializeable for CharacterInfo {
    ...
    fn serialize(&self) -> Vec<crate::data::EOByte> {
        let mut builder =
		StreamBuilder::with_capacity(CHARACTER_INFO_SIZE + self.name.len() + 1);
...
#[cfg(test)]
mod tests {
    use crate::{data::{StreamReader, EOByte, Serializeable}, character::{Gender, Race, AdminLevel}, net::PaperdollBAHSW};
    ...
    fn deserialize() {
		let buf: Vec<EOByte> = vec![
            103, 111, 114, 111, 110, 255, 182, 73, 3, 254, 42, 2, 25, 4, 1, 1, 53, 254, 49, 254, 34, 254, 17, 254, 74, 254
        ];
